...
| 构建 | Maven（离线 `-o` 模式依赖 framework4j v1.5.1 已入本机 m2） |
...
# 产物：notification4j-app/target/notification4j-app-1.0.0.jar（前端 SPA 已入 jar，免双部署）
```
...
- **安全面出厂默认**（v1.2.2）：runtime 不强制签名（`path-patterns=[]`，S2S 需要时自行加回）、druid `filters: stat,slf4j`（wall 不兼容 PG SKIP LOCKED）、redisson-spring-boot-starter 已排除。详见 `backend/README.md` 安全面节与 `docs/api/api-spec.md` V1.2.2 修订。
- 生产建议：反向代理 TLS + 进程托管（systemd/K8s）；`backend/bin/run.sh` 含 G1GC 参考参数。

## 4. 形态二：嵌入业务方应用（starter，进程内直调）

引 `fun.commons.notification4j:notification4j-starter:1.0.0`（坐标版本以本地 m2 为准），
