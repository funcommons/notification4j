...
> 按《开发原则》§2.4，以上决策与任何「参考工程原样复用（adapter/）」均需在 `documents/adr/` 留决策记录。
...
| 终端用户 | `ROLE_END_USER` | 查看自己的消息与公告、注册自己的站外渠道、配置订阅偏好 | 嵌入消息中心（消息/公告/渠道/订阅 4 页） | 无（仅操作自己 `X-User-Id` 名下数据） |
...
- 免打扰时段：V1.2 预留 `quiet_hours` 字段，V1.0 不实现 UI。
...
| 2 | /nfy/api/v1/admin/types/{type_id} | PATCH/DELETE | 修改/停用 | name, description, default_level, default_channels, status |
...
- **渠道隔离**：每渠道类型独立线程池；按渠道实例（channel_id）集中限速（钉钉/企微 20 条/分钟/机器人，fwk4j-rate-limit 编程式 Redis 限速，多实例安全），单渠道故障/缓慢不影响其他渠道；
...
| 布局 | 路由前缀 | 形态 | 适用 |
|---|---|---|---|
| 完整 app | `/nfy/tenant/app/*` | 带侧栏导航（消息/公告/渠道/订阅 4 页） | 接入方嵌入完整消息中心 |
| 单页 page | `/nfy/tenant/page/*` | 无壳纯内容 | 嵌铃铛 `/bell`、消息列表 `/messages` 等单页 |

平台端同理 `/nfy/platform/app|page/*`。View 组件完全复用，仅外层 layout 不同。

#### 2. 认证两级
- 基础级：URL `?access_token=`（内网快速验证）；
...
| 签名防重放 | runtime 发送面 HMAC（framework4j-signature）**按租户开关**（`privileges.signature`）：未开启不校验；开启后密钥未注册=全拒（安全默认）、已注册=强制校验（缺失/过期/重复/无效四态错误码）；签名密钥注册/轮换走 admin 域端点（ADR-0008） |
...
| 外发吞吐 | 单渠道 ≥ 20 msg/min（IM 限额打满），邮件 ≥ 100/min | 渠道独立线程池 + 限速器 |
...
| **V1.2** | 短信渠道（Sms4J）、免打扰时段（quiet_hours）、消息撤回、人工重投 UI 增强 | F-CHN+SMS、F-SUB quiet_hours |
...
### 7.3 引用文档
- 《开发原则》（项目根目录）
- 《中间件中台租户设计》（项目根目录，benefit4j 实战抽象 v2.1）
- 《双模式技术方案》（项目根目录，v2.0.1）
