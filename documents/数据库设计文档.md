...
| V1.0.0 | 2026-09-16 | 新建 | 初始化 11 张表（10 业务表 + 1 审计表），Flyway 脚本 `V202609161000__init_nfy_schema.sql` | justin | 待评审 |
...
1. **禁外键 / 禁触发器 / 禁存储过程 / 禁 SELECT \***（mc-database-spec 铁律）；
...
|---|---|---|---|---|---|---|
| nfya_tenant | uk_nfya_tenant_email | email | 部分唯一 B-Tree | 租户查重 | 100% | `WHERE is_deleted=0 AND email<>''` |
...
└── V1.0.0__init_nfy_schema.sql             -- 本文档 §9.1 全量 DDL（已落盘；starter 模块承载，app 合并 classpath 可见）
```
...
### 9.1 完整 DDL（V1.0，与 Flyway `V202609161000__init_nfy_schema.sql` 一一对应）

```sql
-- =============================================================================
-- NFY (Notification Middle-platform) Database Schema  — V1.0.2
-- 内容: nfya_* 业务表 10 张 + nfyp_registration_key 1 张; nfyp_audit_log 由 framework4j-audit 自动建表(不在此脚本)
-- 规约: 无外键/无触发器/无存储过程; 参与查询过滤的 JSONB 列建 GIN(亿级写表 ext 不建); 唯一约束部分唯一(is_deleted=0)
-- 生成: 本 DDL 待首个实施任务落为 V202609161000__init_nfy_schema.sql 并回校本文档(评审第3轮措辞修正)
-- 变更: V1.0.1 评审第1轮修复(delivery幂等闸/next_retry_at默认now/注册码表/secret加宽/订阅去version/公告biz_no)
-- =============================================================================
...
CREATE UNIQUE INDEX uk_nfya_tenant_email ON nfya_tenant (email) WHERE (is_deleted = 0 AND email <> '');
...
- 《产品设计文档（PRD）》documents/产品设计文档.md（功能与分期来源）
