...
| V1.0.1 | 2026-09-16 | 变更 | 评审第 1 轮修复：补嵌入形态全组合矩阵与能力边界；出站限速改 fwk4j-rate-limit 集中式；签名按租户开关（ADR-0008）；ADR-0001~0008 落盘 documents/adr/；引擎循环语义闭环（SENDING reaper/at-least-once）；增设 notification4j-common | justin | 评审会（第 1、2 轮） |
...
|---|---|
| 定向站内消息、平台/租户公告、三 IM webhook 渠道 + 邮件 SMTP 渠道、订阅偏好、外发投递状态机、嵌入消息中心、租户接入全套 | App 厂商通道推送、工作流编排、A/B 测试、digest、语音、微信系、短信（V1.2，PRD §5.3 不做清单） |
...
| P7 | **配置驱动双模式** | `nfy.notification.mode` / `enable-api` 两开关切三形态（《双模式技术方案》） |
...
graph LR
    ...
    subgraph SVC[notification4j]
        ...
        FE[Vue 3 SPA<br/>控制台 + 嵌入页 /nfy/{platform|tenant}/{app|page}/*]
    end
    subgraph DATA[数据层]
...
graph LR
    ...
    DISP --> P3[飞书线程池]
    DISP --> P4[邮件线程池]
    P1 --> RL[fwk4j-rate-limit 集中限速<br/>key=channel_id]
    P2 --> RL
    P3 --> RL
    P4 --> RL
    RL --> S[ChannelSender SPI]
    S -->|渠道业务码成功| OK[置 SUCCESS<br/>渠道 fail_count 清零]
...
> **语义声明**：at-least-once（reaper 回收或崩溃重投可能致渠道侧重复，可接受）；成功判定=渠道业务码（钉钉/企微 errcode=0、飞书 code=0、SMTP 2xx）；两类计数器分离——投递行 `retry_count`（单任务重试）vs 渠道 `fail_count`（熔断）。
...
├── notification4j-common/                ← 公共层：model/util/constant + 外发引擎（scanner/dispatcher/ChannelSender SPI + 4 渠道实现，自建共享件统一归口《开发原则》§1.2；下沉 common 使「嵌入自跑引擎」形态可装配）
├── notification-spring-boot-starter/     ← 业务方唯一依赖（对应 benefit4j-starter）
│   ├── config/NfyProperties              ← @Validated: mode/enable-api/remote-url/超时
│   ├── client/NotifyClient               ← 门面接口：send/sendBatch/announce/queryResult
│   ├── client/impl/LocalNotifyClient     ← mode=local，直连 NfyService
│   ├── client/impl/RemoteNotifyClient    ← mode=remote，HttpTransport + JWT + Idempotency-Key
│   ├── service/NfyService                ← 本地业务接口
│   ├── service/impl/DefaultNfyService    ← ★ 默认实现下沉 starter（MyBatis-Plus + nfya_*，数据源指向中台库）
│   ├── controller/                       ← @ConditionalOnProperty enable-api=true
│   ├── transport/HttpTransport           ← RestTemplate 缺省，可替换
│   └── autoconfigure/NfyAutoConfiguration
├── notification4j-app/                   ← 独立服务入口（对应 benefit4j-app）
│   ├── controller/（三域）
│   └── domain/（message/announcement/channel/subscription/delivery 业务装配）
└── notification4j-it/                    ← 集成测试（Testcontainers，越权/幂等/外发全链路）
```

> **与《双模式技术方案》7 模块的对应**：单业务域（N=1）下 3 个域 starter 收敛为 1 个；common 保留（自建件归口）；app/it 不变。偏离已记录于 ADR-0001。

**装配条件矩阵**（《双模式技术方案》§7 实例化）：

| Bean | 条件 |
|---|---|
| LocalNotifyClient | `nfy.notification.mode=local`（默认）+ NfyService 存在，缺失启动即失败 |
| RemoteNotifyClient | `mode=remote` + `Assert.hasText(remote-url)` |
| 三域 Controller | `nfy.notification.enable-api=true`（**默认值分侧**：nfy-app 默认 true；starter 侧默认 false，防嵌入方进程意外暴露 HTTP 面） |
| 外发引擎 | `nfy.engine.enabled=true`（nfy-app 默认开；嵌入形态默认关） |
...
| 签名防重放 | runtime 发送面 HMAC（fwk4j-signature）**按租户开关**（`privileges.signature`，ADR-0008）：未开启=不校验；开启后密钥未注册=全拒（对齐《租户设计》安全默认）、已注册=强制校验（四态错误码，常量时间比较）；密钥注册/轮换走 admin 域 API-SEC-001 |
...
sequenceDiagram
    ...
    E->>RD: 集中限速 tryAcquireFixedWindow(channel_id)
    E->>X: 渠道适配器发送(加签)
    X-->>E: 业务码成功 / 失败
...
**限速实现**：按渠道实例集中限速——`fwk4j-rate-limit` 编程式固定窗口 `tryAcquireFixedWindow("nfy:ch:"+channel_id, limit, 60)`（Redis Lua 分布式，多实例合计不超限额；评审第 1 轮修正：弃用 Guava 本地限速——双实例会使 20/min 变 40/min 超钉钉/企微硬限额）。**窗口边界突刺说明**：固定窗口在交界处 60s 滑动口径最高放 2×limit，15/min（75%）档**降低越限概率**（需相邻两窗同时打满才越限，常态流量概率极低）；如需硬保证不超 20/min，配置 50% 档（10 条/min）——两档皆可配，默认 75% 档；fwk4j 编程式若提供滑动窗口 API 则优先切换（实施时确认）。群机器人限额按机器人算，同群多渠道共用时由用户自担（文档提示）。
...
**熔断**：以**每次投递失败（置 FAILED）**为触发原子计数（`UPDATE nfya_channel SET fail_count = fail_count + 1 ...`，成功清零）——不等 DEAD（按死信计会拖慢熔断约 4 倍，评审第 3 轮统一口径，§4.3 图 CB 边挂在 FAILED 节点）；`fail_count≥5` → DISABLED + 给属主站内信；重新启用必须再验证（状态机见 PRD §2.7）。注意区分两个计数器：投递行 `retry_count`（单任务重试次数）与渠道 `fail_count`（连续失败熔断计数）。
...
| 入口 | 路由 | 用途 |
|---|---|---|
| 租户完整壳 | `/nfy/tenant/app/{messages,announcements,channels,subscriptions}` | 完整消息中心 |
| 租户单页 | `/nfy/tenant/page/{bell,messages,...}` | iframe 嵌单页/铃铛 |
| 平台壳/单页 | `/nfy/platform/{app,page}/*` | 平台运营 |
...
| 限流 | fwk4j-rate-limit | §2.6 五维限流（接口文档）+ **外发出站集中限速**（编程式 tryAcquireFixedWindow，key=channel_id） |
...
> 缺口处理：外发引擎的「DB 队列 + SKIP LOCKED 调度」framework4j 无对应模块——按 §1.2 路径 1 自建，放 `notification4j-common/engine` 统一维护（评审第 3 轮下沉，支撑嵌入自跑形态；DB 访问经接口注入：common 定义 DeliveryStore port，starter/app 装配 MyBatis-Plus 实现），已留 ADR-0003 与 issue（演进为 MQ 时替换）。

### 5.9 自建件清单（全部经确认留痕）

| 自建件 | 原因 | 位置 | ADR |
|---|---|---|---|
| 外发调度引擎 | fwk4j 无任务队列模块 | notification4j-common/engine | ADR-0003 |
| 渠道适配器 4 实现（含 SPI 接口） | 业务域特有 | notification4j-common/engine（下沉 common 使嵌入自跑形态可装配） | ADR-0004 |
| 未读数缓存维护逻辑 | 业务规则 | starter/cache | ADR-0005 |
...
nfy:
  notification:
    mode: ${NFY_MODE:local}                 # local|remote
    enable-api: ${NFY_ENABLE_API:true}      # ★ 本默认仅属 nfy-app；starter 侧默认 false（嵌入方进程不暴露 HTTP 面）
    remote-url: ${NFY_REMOTE_URL:}
  engine:
    enabled: true
    scan-interval-ms: 5000
    batch-size: 200
    retry-delays: [60, 300, 900]            # 退避 1/5/15min
    channel-circuit-threshold: 5
framework4j:
...
> 全部已落盘 `documents/adr/ADR-XXXX-*.md`（含上下文/备选/决策/后果四要素，mc-doc-arch 场景三模板），状态随评审结论流转 proposed → accepted。
...
- 《产品设计文档（PRD）》/《数据库设计文档（DBD）》/《接口设计文档》（documents/）
- 《中间件中台租户设计》v2.1 /《双模式技术方案》v2.0.1 /《开发原则》
